...
import { Text, Flex, Skeleton, Image } from '@quickswap-core/uikit'
  ...
  useEffect(() => {
    const fetchBurnedCakeAmount = async () => {
      try {
        ...
        console.error('Failed to fetch burned auction quick', error)
      }
    }
  ...
  return (
    <Flex flexDirection={['column-reverse', null, 'row']}>
      <Flex flexDirection="column" flex="2">
        {burnedCakeAmount !== 0 ? (
          <Balance fontSize="64px" bold value={burnedCakeAmount} decimals={0} unit=" QUICK" />
        ) : (
      ...
      <Image width={350} height={320} src="/images/burnt-quick.png" alt={t('Burnt QUICK')} />
    </Flex>
  )
}

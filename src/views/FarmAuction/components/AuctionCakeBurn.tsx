...
import { Text, Flex, Skeleton, Image } from '@lenoswap/uikit'
  ...
  useEffect(() => {
    const fetchBurnedCakeAmount = async () => {
      try {
        ...
        console.error('Failed to fetch burned auction leno', error)
      }
    }
  ...
  return (
    <Flex flexDirection={['column-reverse', null, 'row']}>
      <Flex flexDirection="column" flex="2">
        {burnedCakeAmount !== 0 ? (
          <Balance fontSize="64px" bold value={burnedCakeAmount} decimals={0} unit=" LENO" />
        ) : (
      ...
      <Image width={350} height={320} src="/images/burnt-leno.png" alt={t('Burnt LENO')} />
    </Flex>
  )
}
